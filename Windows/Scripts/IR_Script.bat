...
::						add: 	acquire memory
::						add: 	collect schtasks Tasks folder
:: 						add: 	collect prefetch files
:: 						modify: collect all winevt logs by robocopy
::						modify: collect each user's ntuser.dat, usrclass.dat, and recent folder
::	     	 Dec 2020 - add: 	create collection folder automatically
::						modify:	collect each fixed disk's MFT
::			 JAN 2021 - add: 	collect bitmap cache
::						add: 	collect shellbags
::						add: 	collect Windows.edb
::						add: 	collect Recycle Bin files
::						add: 	collect Amcache.hve
::						add: 	collect SRUM folder
::			 Aug 2021 - add:	define legacy platform collection procedure
::			 Sep 2021 - add:	collect Antivirus logs, browsing history, web server logs, powershell console logs, FTP related logs
:: 			 Nov 2021 - add:	collect Win10 Timeline ActivitiesCache.db
::			 Sep 2022 - add: 	add Collection.log
::			 		  - modify: bug fixes
::			 Nov 2022 - modify: bug fixes
:: --------------------------------------------------------------------------------------------------------------------------
:: --------------------------------------------------------------------------------------------------------------------------
...
if not exist %COLLECTION_FOLDER% (
	...
	echo --------------------------------------------------------------------------------------------------------------------
	dir %SYSTEM_DRIVE%\ /a/s/od/ta > %COLLECTION_FOLDER%\Timeline\%COMPUTERNAME%_AccessTime.txt
	dir %SYSTEM_DRIVE%\ /a/s/od/tc/q > %COLLECTION_FOLDER%\Timeline\%COMPUTERNAME%_CreationTime.txt
	dir %SYSTEM_DRIVE%\ /a/s/od/tw > %COLLECTION_FOLDER%\Timeline\%COMPUTERNAME%_WriteTime.txt
	...
	echo --------------------------------------------------------------------------------------------------------------------
	echo %DATE% %TIME% - Collecting Each Fixed Disk's MFT and parsing to csv
	echo --------------------------------------------------------------------------------------------------------------------
	...
	echo [+] %DATE% %TIME% - Collecting Sign InformSation >> %COLLECTION_FOLDER%\Collection.log
	echo --------------------------------------------------------------------------------------------------------------------
	echo %DATE% %TIME% - Collecting Sign InformSation
	echo --------------------------------------------------------------------------------------------------------------------
	"%COLLECTFILESTOOLS_FOLDER%\signtool\x64\signtool.exe" verify /pa /q C:\Windows\*.exe 2> %COLLECTION_FOLDER%\SignInfo\%COMPUTERNAME%_Windows_exe.txt
	"%COLLECTFILESTOOLS_FOLDER%\signtool\x64\signtool.exe" verify /pa /q C:\Windows\*.dll 2> %COLLECTION_FOLDER%\SignInfo\%COMPUTERNAME%_Windows_dll.txt
	"%COLLECTFILESTOOLS_FOLDER%\signtool\x64\signtool.exe" verify /pa /q C:\Windows\System32\*.exe 2> %COLLECTION_FOLDER%\SignInfo\%COMPUTERNAME%_System32_exe.txt
	"%COLLECTFILESTOOLS_FOLDER%\signtool\x64\signtool.exe" verify /pa /q C:\Windows\System32\*.dll 2> %COLLECTION_FOLDER%\SignInfo\%COMPUTERNAME%_System32_dll.txt
	"%COLLECTFILESTOOLS_FOLDER%\signtool\x64\signtool.exe" verify /pa /q C:\Windows\syswow64\*.exe 2> %COLLECTION_FOLDER%\SignInfo\%COMPUTERNAME%_Syswow64_exe.txt
	"%COLLECTFILESTOOLS_FOLDER%\signtool\x64\signtool.exe" verify /pa /q C:\Windows\syswow64\*.dll 2> %COLLECTION_FOLDER%\SignInfo\%COMPUTERNAME%_Syswow64_dll.txt
	...
	%COLLECTFILESTOOLS_FOLDER%\Robocopy\Robocopy.exe %SYSTEM_DRIVE%\Windows\system32\sru %COLLECTION_FOLDER%\SRUM /ZB /copy:DAT /r:0 /ts /FP /np /E
	...
	REM WinSCP ini file
	%COLLECTFILESTOOLS_FOLDER%\Robocopy\Robocopy.exe %SYSTEM_DRIVE%\Windows\ %COLLECTION_FOLDER%\FTP WinSCP.ini
